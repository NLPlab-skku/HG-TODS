...
class DecoderLayer(nn.Module):
    def __init__(self, cfg):
        ...
        ### gating
        self.experts_fc1 = nn.ModuleList([nn.Linear(self.d_model, 4 * self.d_model) for i in range(2)])
        self.experts_fc2 = nn.ModuleList([nn.Linear(4 * self.d_model, self.d_model) for i in range(2)])
        self.w_gate = nn.Linear(self.d_model, 2)
        self.w_noise = nn.Parameter(torch.zeros(1, 2), requires_grad=True)
    ...
    def forward(
        ...
        residual = hidden_states
        _enc_hidden_states, enc_attn_present_key_value = self.encoder_attn(
            query_states = hidden_states,
        ...
        _enc_hidden_states = self.encoder_attn_layer_norm(_enc_hidden_states)
        ...
        _graph_hidden_states = self.graph_attn_layer_norm(_graph_hidden_states)

        # hidden_states = residual + _enc_hidden_states + _graph_hidden_states
        
        # _enc_hidden_states = _enc_hidden_states * gating[:,:,0].unsqueeze(2).repeat(1,1,self.d_model)
        # _graph_hidden_states = _graph_hidden_states * gating[:,:,1].unsqueeze(2).repeat(1,1,self.d_model)
        # hidden_states = residual + _enc_hidden_states + _graph_hidden_states 
        
        residual = hidden_states
        ### gating (토큰별)
        gating = self.w_gate(hidden_states) + self.w_noise
        ### gating
        gating = F.softmax(gating, dim=2)
        ...
        enc_hidden_states = self.activation_fn(self.experts_fc1[0](_enc_hidden_states))
        enc_hidden_states = F.dropout(enc_hidden_states, p = self.activation_dropout, training = self.training)
        enc_hidden_states = self.experts_fc2[0](enc_hidden_states)
        enc_hidden_states = F.dropout(enc_hidden_states, p=self.dropout, training=self.training)

        graph_hidden_states = self.activation_fn(self.experts_fc1[1](_graph_hidden_states))
        graph_hidden_states = F.dropout(graph_hidden_states, p = self.activation_dropout, training = self.training)
        graph_hidden_states = self.experts_fc2[1](graph_hidden_states)
        graph_hidden_states = F.dropout(graph_hidden_states, p=self.dropout, training=self.training)

        enc_hidden_states = enc_hidden_states * gating[:,:,0].unsqueeze(2).repeat(1,1,self.d_model)
        graph_hidden_states = graph_hidden_states * gating[:,:,1].unsqueeze(2).repeat(1,1,self.d_model)

        hidden_states = residual + hidden_states + enc_hidden_states + graph_hidden_states
        hidden_states = self.final_layer_norm(hidden_states)
        ...
        present_key_value = self_attn_present_key_value + enc_attn_present_key_value + graph_attn_present_key_value
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_ids, dec_mask = None, enc_hidden_states = None, enc_mask = None, graph_hidden_states = None, graph_mask = None, past_key_values = None):
...
class BartModel(nn.Module):
    ...
    def forward(self, enc_ids, enc_mask, g, dec_ids, dec_mask = None, enc_hidden_states = None, graph_hidden_states = None, graph_mask = None, past_key_values = None):
...
class GraphBartForConditionalGeneration(nn.Module):
    def __init__(self, cfg, tokenizer):
        super().__init__()
        self.cfg = cfg
        self.tokenizer = tokenizer
    ...
    def forward(self, enc_ids, enc_mask, g, dec_ids, dec_mask = None, enc_hidden_states = None, graph_hidden_states = None, graph_mask = None, past_key_values = None, label_ids = None):
        transformer_outputs = self.model(enc_ids, enc_mask, g, dec_ids, dec_mask, enc_hidden_states, graph_hidden_states, graph_mask, past_key_values)
    ...
    def generate(self, enc_ids, enc_mask, g = None):
        ...
        for _ in range(self.cfg.generate_max_length):
            model_outputs = self.forward(
                enc_ids, enc_mask, g, dec_ids, dec_mask, enc_hidden_states, graph_hidden_states, graph_mask, past_key_values = past_key_values
            )
