...
class DecoderLayer(nn.Module):
    def __init__(self, cfg):
        ...
        self.cgraph_attn = MultiHeadAttention(cfg, is_decoder = True, is_cross_attention = True)
        self.cgraph_attn_layer_norm = nn.LayerNorm(self.d_model)
    ...
    def forward(
        ...
        graph_dec_mask = None, 
        cgraph_hidden_states = None,
        cgraph_dec_mask = None, 
        past_key_value = None
    ):
        self_attn_past_key_value = past_key_value[:2] if past_key_value is not None else None
        enc_attn_past_key_value = past_key_value[2:4] if past_key_value is not None else None
        graph_attn_past_key_value = past_key_value[4:6] if past_key_value is not None else None
        cgraph_attn_past_key_value = past_key_value[6:] if past_key_value is not None else None
        ...
        residual = hidden_states
        _cgraph_hidden_states, cgraph_attn_present_key_value = self.cgraph_attn(
            query_states = hidden_states,
            key_value_states = cgraph_hidden_states,
            past_key_value = cgraph_attn_past_key_value,
            attention_mask = cgraph_dec_mask,
        )
        _cgraph_hidden_states = F.dropout(_cgraph_hidden_states, p = self.dropout, training = self.training)
        _cgraph_hidden_states = self.cgraph_attn_layer_norm(_cgraph_hidden_states)

        hidden_states = residual + _enc_hidden_states + _graph_hidden_states + _cgraph_hidden_states
        ...
        present_key_value = self_attn_present_key_value + enc_attn_present_key_value + graph_attn_present_key_value + cgraph_attn_present_key_value 
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_ids, dec_mask = None, enc_hidden_states = None, enc_mask = None, graph_hidden_states = None, graph_mask = None, cgraph_hidden_states = None, cgraph_mask = None, past_key_values = None):
        ...
        cgraph_dec_mask = _expand_mask(cgraph_mask, dec_ids.shape[-1])
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            layer_outputs = decoder_layer(hidden_states, dec_self_mask, enc_hidden_states, enc_dec_mask, graph_hidden_states, graph_dec_mask, cgraph_hidden_states, cgraph_dec_mask, past_key_value)
...
class BartModel(nn.Module):
    def __init__(self, cfg, tokenizer):
        ...
        if cfg.method == 'BART-gnn-gat':
            self.gnn = GAT(cfg)
        elif cfg.method == 'BART-gnn-gcn':
            self.gnn = GCN(cfg)
        elif cfg.method == 'BART-gnn-rgat':
            self.gnn = RGAT(cfg)
        if cfg.method == 'BART-gnn-dualrgat':
            self.gnn = RGAT(cfg)
            self.cgnn = RGAT(cfg)
    ...
    def load_plm(self):
        state_dict = torch.load('/workspace/NRF/cache/graph_kobart')
    ...
    def forward(self, enc_ids, enc_mask, g, cg, dec_ids, dec_mask = None, enc_hidden_states = None, graph_hidden_states = None, graph_mask = None, cgraph_hidden_states = None, cgraph_mask = None, past_key_values = None):
        if enc_hidden_states is None and graph_hidden_states is None:
            ...
            cg = self.cgnn(cg)
            cgraph_hidden_states, cgraph_mask = self.mk_hidden(cg)


        dec_outputs = self.decoder(dec_ids, dec_mask, enc_hidden_states, enc_mask, graph_hidden_states, graph_mask, cgraph_hidden_states, cgraph_mask, past_key_values)

        return {
            'enc_hidden_states' : enc_hidden_states,
            'graph_hidden_states' : graph_hidden_states,
            'graph_mask' : graph_mask,
            'cgraph_hidden_states' : cgraph_hidden_states,
            'cgraph_mask' : cgraph_mask,
            'dec_hidden_states' : dec_outputs['dec_hidden_states'],
            'past_key_values' : dec_outputs['past_key_values']
        }

class DualGraphBartForConditionalGeneration(nn.Module):
    def __init__(self, cfg, tokenizer):
        super().__init__()
        self.cfg = cfg
        self.tokenizer = tokenizer
    ...
    def forward(self, enc_ids, enc_mask, g, cg, dec_ids, dec_mask = None, enc_hidden_states = None, graph_hidden_states = None, graph_mask = None, cgraph_hidden_states = None, cgraph_mask = None, past_key_values = None, label_ids = None):
        transformer_outputs = self.model(enc_ids, enc_mask, g, cg, dec_ids, dec_mask, enc_hidden_states, graph_hidden_states, graph_mask, cgraph_hidden_states, cgraph_mask, past_key_values)
        ...
        return {
            ...
            'graph_mask' : transformer_outputs['graph_mask'],
            'cgraph_hidden_states' : transformer_outputs['cgraph_hidden_states'],
            'cgraph_mask' : transformer_outputs['cgraph_mask'],
    ...
    def generate(self, enc_ids, enc_mask, g = None, cg = None):
        ...
        for _ in range(self.cfg.generate_max_length):
            model_outputs = self.forward(
                enc_ids, enc_mask, g, cg, dec_ids, dec_mask, enc_hidden_states, graph_hidden_states, graph_mask, cgraph_hidden_states, cgraph_mask, past_key_values = past_key_values
            )
            ...
            cgraph_hidden_states = model_outputs['cgraph_hidden_states']
            cgraph_mask = model_outputs['cgraph_mask']
