...
class GraphTransformer(nn.Module):
    def __init__(self, cfg, tokenizer):
        super().__init__()
        self.embed_tokens = nn.Embedding(len(tokenizer), cfg.d_model, tokenizer.pad_token_id)
        self.embed_positions = PositionalEncoding(cfg.d_model, cfg.max_position_embeddings)
...
class GraphTransformerForConditionalGeneration(nn.Module):
    def __init__(self, cfg, tokenizer):
        super().__init__()
        self.cfg = cfg
        self.tokenizer = tokenizer
        self.model = GraphTransformer(cfg, tokenizer)
    ...
    def forward(
        ...
        model_outputs = self.model(
            enc_ids, enc_mask, g, dec_ids, dec_mask, enc_hidden_states, graph_hidden_states, graph_mask, past_key_values
        )
        ...
        if label_ids is not None:
            ...
            lm_loss = criterion(lm_logits.view(-1, len(self.tokenizer)), label_ids.view(-1))
